...
from tamer_updates import get_greedy_action, update_reward_model_with_credit
...
if __name__ == "__main__":
    ...
    itraj = 0
    while True:
        saveObject(theta, 'trained_tamer_async.pkl')
        saveObject([inputs, targets], 'dataset_tamer_async.pkl')
        print("Saved trained_tamer_async.pkl with dataset so far in dataset_tamer_async.pkl")
        ...
        for t in range(T):
            act, act_val = get_greedy_action(obs, [a for a in ACT_DICT.keys()], theta)
            was_feedback_provided = False

            print(f"t={t:2d}: Action {ACT_DICT[act]:5s} with val {act_val: 4.1f}. Feedback? ",
                  end="", flush=True)
            ...
            fb_val = 0
            while pygame.time.get_ticks() - tick_start < 1000:
            ...
            if was_feedback_provided:
                credit_weights = np.array([0.1, 0.1, 0.8])
                theta = update_reward_model_with_credit(obs_list, act_list, theta, fb_val, credit_weights, learning_rate)

...
class IRLEnv(FrozenLakeEnv):
    ...
    def __init__(
        ...
        self.observation_space = spaces.Box(-1, 1, [dummy_obs.shape[0]])
    ...
    def monster_agent_crossover(self, s, m, agent_action, monster_action):
        # Returns true if the monster and the agent are in adjacent cells and move
        # in opposite directions (i.e., there is a crossover).
        # In this case, it is considered equivalent to being eaten by the monster.

        # Convert grid cell to (row,col)
        monster_loc = (m//self.nrow, m%self.nrow)
        agent_loc = (s//self.nrow, s%self.nrow)

        if monster_loc[0] == agent_loc[0] and abs(monster_loc[1] - agent_loc[1]) == 1:
            if agent_action == 0 and monster_action == 2 or \
                    agent_action == 2 and monster_action == 0:
                # print("CROSSOVER")
                return True

        elif monster_loc[1] == agent_loc[1] and abs(monster_loc[0] - agent_loc[0]) == 1:
            if agent_action == 1 and monster_action == 3 or \
                    agent_action == 3 and monster_action == 1:
                # print("CROSSOVER")
                return True
        else:
            # Agents not in adjacent grid to monster. No crossover
            return False
    ...
    def step(self, action, monster_action=None):
        ...
        # if s == m:
        # if s == m or self.monster_agent_crossover(s, m, agent_action=action, monster_action=monster_action):
        if s == m or s == self.m:
            # if eaten by monster
        ...
        return self.get_obs_vector(s, m), r, t, False, {"prob": p}
    ...
    def reset(
        ...
        if self.render_mode == "human":
            self.render()
        return self.get_obs_vector(self.s, self.m), {"prob": 1}
